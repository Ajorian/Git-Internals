...
````
In the command mentioned above, COMMIT_HASH is a SHA-1 hash of the commit object generated by the commit-tree command. The resulting state is also illustrated in the following picture: 
...
````
Note that -p flag in the last command sets a parent for this new commit. The resulting state is as follows:
